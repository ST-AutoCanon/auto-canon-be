  ...
  try {
    ...
    if (data.Front_Wheel_Rim) {
      ...
      if(item != null) {await form8Schema.findByIdAndUpdate(form8._id,
          {$set: {
              "Wheel_Rim.WheelRim.$[item].Front_Wheel_Rim.properties.Make.value": data.Front_Wheel_Rim.properties.Make.value,
            },
          },{ arrayFilters: [{ "item._id": item._id }]})
      }
      }
...
if (data.General_arrangement_vehicle) {
  ...
  if (form7 != null) {
    ...
    if(item != null) {await form7Schema.findByIdAndUpdate(form7._id,
        {$set: {
            "General_arrangement_of_the_vehicle.Generalarrangementofthevehicle.$[item].General_arrangement_vehicle.properties.Number_of_seating_positions.value": data.General_arrangement_vehicle.properties.Number_of_seating_positions.value,
          },
        },{ arrayFilters: [{ "item._id": item._id }]})
    }
  }
...
if (data.Steering_System) {
  ...
  if (form7 != null) {
    const item = form7.Steering.Steering.find(arrItem => arrItem.supplier && arrItem.supplier.toString() === data.supplier._id);
    if(item != null) {await form7Schema.findByIdAndUpdate(form7._id,
        {$set: {
            "SteeringSuspensionAntiTheft.SteeringSuspensionAntiTheftData.$[item].Steering_System.properties.Select_Type_Steering_control_provided.value": data.Steering_System.properties.Steering_control_provided.value,
            "SteeringSuspensionAntiTheft.SteeringSuspensionAntiTheftData.$[item].Steering_System.properties.Specify_steering_gear_ratio.value": data.Steering_System.properties.steering_gear_ratio.value,
          },
        },{ arrayFilters: [{ "item._id": item._id }]})
    }
  }
...
if (data.DriveTrainSystem) {
  updatedform1AData = await form1ASchema.findByIdAndUpdate(
    form1A._id,
    {
      $set: {
        "DriveTrainSystem.DriveTrainSystemData.$[driveTrainSystemData].Transmission": data.DriveTrainSystem,
      },
    },
